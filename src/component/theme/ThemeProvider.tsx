...
//            v4: Now wrapped in withoutTransitions() for visual
//            consistency with the manual toggle. Also guarded by
//            explicitChoiceRef to eliminate a micro-race where an
//            OS change event could slip between applyTheme and
//            writeStoredTheme during a manual toggle.
//
//   BUG #4 → Cross-tab sync via storage event. Toggle in one tab,
//            all other tabs update instantly. next-themes does this.
//            v4: Now wrapped in withoutTransitions() so cross-tab
//            updates snap instantly instead of animating.
//
...
//            v4: Now bails out early if resolved theme matches
//            current DOM — prevents unnecessary repaints. Wrapped
//            in withoutTransitions() when a change is needed.
//
...
//            at different rates.
//            v4: Tokenized cleanup — each toggle increments a
//            module-scope counter. Cleanup callbacks only fire if
//            their token matches the current counter, preventing
//            spam-click races where an older timer removes the
//            style tag during a newer toggle's suppressed window.
//            Also clears the previous setTimeout to avoid orphans.
//
//   FIX E  → Meta theme-color synced on toggle. Uses cached
...
//            v4: Meta elements cached at module level — no
//            repeated DOM queries on subsequent toggles.
//
...
//
//   FIX H  → React state initialized from DOM, not hardcoded.
//            The blocking script in layout.tsx has already set
//            data-theme before React initializes.
//
//   FIX I  → (v4) Removed syncColorSchemeMeta entirely.
//            The <meta name="color-scheme"> is now static
//            "light dark" in layout.tsx — declares support for
//            both schemes. CSS drives the active one via
//            [data-theme="dark"] { color-scheme: dark }. This
//            eliminates a function, 5 call sites, and a blocking
//            script line. Fewer DOM writes, more spec-correct.
//
//   FIX J  → (v4) Explicit-choice ref. A useRef<boolean> flag
//            set synchronously at the start of toggleTheme and
//            checked by the system-change handler. Eliminates a
//            micro-race where an OS theme change event could fire
//            between applyTheme() and writeStoredTheme(), see no
//            stored theme, and override the user's toggle.
//
//   FIX K  → (v4) visibilitychange safety net. If the user
//            toggles theme and immediately backgrounds the tab,
//            Chrome throttles both rAF and setTimeout in background
//            tabs. The no-transition <style> tag could stay
//            injected for minutes, suppressing all CSS transitions.
//            A visibilitychange listener force-removes the orphaned
//            tag when the tab returns to foreground.
//
// ═══════════════════════════════════════════════════════════════
"use client";
