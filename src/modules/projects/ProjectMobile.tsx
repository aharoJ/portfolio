...
export default function ProjectMobile() {
  ...
  return (
    <section aria-label="Projects (mobile)" className="w-full bg-neon-default">
      <div className="mx-auto max-w-2xl px-5 py-10">
        {/* Filter — horizontally scrollable chips, no scrollbar */}
        <nav
          aria-label="Filter projects by category"
          className="-mx-5 mb-6 flex snap-x snap-mandatory gap-2 overflow-x-auto px-5 [scrollbar-width:none] [-ms-overflow-style:none]"
        >
          <style jsx global>{`
            nav[aria-label="Filter projects by category"]::-webkit-scrollbar {
              display: none;
            }
          `}</style>
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={[
                  "snap-start shrink-0 rounded-full px-3 py-1.5 text-sm font-semibold transition-colors",
                  on
                    ? "bg-soft-moss text-creamy-bone"
                    : "bg-creamy-bone text-soft-slate active:bg-creamy-ivory",
                ].join(" ")}
                aria-pressed={on}
              >
                {cat}
          ...
          {projects.map((p, idx) => (
            <li key={`${p.title}-${idx}`}>
              <ProjectCardMobile
                ...
                github={p.github}
                hashtags={p.hashtags}
                priority={idx < 2} // only first two eager-load
              />
            </li>
          ))}
        </ul>
